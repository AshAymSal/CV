...
class ProductRepo {
  ...
  Future<ApiResponse> getLatestProductListByBranch(
      BuildContext context, String offset, String languageCode) async {
    try {
      final response = await dioClient.get(
        '${AppConstants.LATEST_PRODUCT_BY_BRANCH}${Provider.of<SplashProvider>(context, listen: false).selectedBranch.id.toString()}?limit=10&&offset=$offset',
        options: Options(headers: {'X-localization': languageCode}),
      );
      print("asdasdasdasdasdasd" + response.toString());
      return ApiResponse.withSuccess(response);
    } catch (e) {
      return ApiResponse.withError(ApiErrorHandler.getMessage(e));
    }
