...
class ProductProvider extends ChangeNotifier {
  ...
  List<Product> _latestProductListByBranch;
  ...
  List<Product> get latestProductListByBranch => _latestProductListByBranch;
  ...
  Future<void> getPopularProductList(BuildContext context,
      String _popularOffset, bool reload, String languageCode) async {
    ...
    if (!_popularOffsetList.contains(_popularOffset)) {
      ...
      ApiResponse apiResponse =
          await productRepo.getPopularProductList(_popularOffset, languageCode);
      if (apiResponse.response != null &&
        ...
        for (Product p in total) {
          if (p.branchId ==
              Provider.of<SplashProvider>(context, listen: false)
                  .selectedBranch
                  .id) {
            _popularProductList.add(p);
          }
        }

        _popularPageSize = total.length;
        _isLoading = false;
      } else {
        _isLoading = false;
  ...
  Future<void> getDailyNeeds(BuildContext context, String _offset, bool reload,
    ...
    if (!_popularOffsetList.contains(_offset)) {
      ...
      _isLoading = true;
      ApiResponse apiResponse =
          await productRepo.getDailyItemList(_offset, languageCode);
      if (apiResponse.response != null &&
          apiResponse.response.statusCode == 200) {
        if (reload || _offset == '1') {
          _dailyItemList = [];
  ...
  Future<void> getLatestProductList(BuildContext context, String _offset,
  ...
  Future<void> getLatestProductListByBranch(BuildContext context,
      String _offset, bool reload, String languageCode) async {
    if (reload || _offset == '1') {
      offset = 1;
      _offsetList = [];
    }
    ...
    if (!_offsetList.contains(_offset)) {
      _offsetList.add(_offset);
      ApiResponse apiResponse = await productRepo.getLatestProductListByBranch(
          context, _offset, languageCode);
      if (apiResponse.response != null &&
          apiResponse.response.statusCode == 200) {
        if (reload || _offset == '1') {
          _latestProductListByBranch = [];
        }
        _latestProductListByBranch
            .addAll(ProductModel.fromJson(apiResponse.response.data).products);
        print(_latestProductListByBranch.length);
        //_latestPageSize = 5;
        _latestPageSize =
            ProductModel.fromJson(apiResponse.response.data).totalSize;
        _isLoading = false;
